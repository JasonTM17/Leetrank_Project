#!/usr/bin/env bash
# typescript_runner.sh — TypeScript code executor for LeetRank judge service.
#
# Usage:
#   bash typescript_runner.sh <code_file_path>
...
if [[ -z "${1:-}" ]]; then
  echo '{"output":"","error":"Usage: typescript_runner.sh <code_file>","timed_out":false}'
  exit 0
fi
...
timeout "$TIMEOUT_SECONDS" ts-node --skip-project "$CODE_FILE" \
