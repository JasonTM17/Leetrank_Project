...
// Mirror the manifest into a smaller flat shape for assertion convenience.
// If the test fails on a future addition we expect that addition to update
// this list explicitly so reviewers see the change.
...
describe("language manifest extra coverage", () => {
  ...
  it("each compiled language declares a non-empty extension starting with a dot", () => {
    for (const lang of LANGUAGE_BY_ID.values()) {
