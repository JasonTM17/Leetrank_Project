...
export default function PasswordSettingsError({
  ...
  useEffect(() => {
    console.error("[/dashboard/settings/password]", error);
  }, [error]);
    ...
    <>
      ...
      <main id="main-content" className="flex-1 flex items-center justify-center px-4 py-16" role="alert" aria-live="assertive">
        <div className="text-center max-w-md animate-fade-in-up">
          ...
          <h1 className="mt-4 text-2xl font-bold">Password settings unavailable</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {error.message || "We couldn't load password settings. Try again in a moment."}
          </p>
          {error.digest && (
